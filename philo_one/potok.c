...
void		ft_thinking(t_data *data, t_philosopher *philosopher)
{
	...
	printf("%ldms \e[%um%i\e[0m is thinking\n", mtime() - data->start_time,
	30 + philosopher->name, philosopher->name);
...
{
	pthread_mutex_lock(&table->forks[max(philosopher->right_fork,
	...
	30 + philosopher->name, philosopher->name);
	pthread_mutex_unlock(&g_entry_point);
	pthread_mutex_lock(&table->forks[min(philosopher->right_fork,
	philosopher->left_fork)]);
	pthread_mutex_lock(&g_entry_point);
...
t_philosopher *philosopher, t_table *table)
{
	pthread_mutex_lock(&philosopher->life_mutex);
	philosopher->life_time = 0;
	pthread_mutex_unlock(&philosopher->life_mutex);
	pthread_mutex_lock(&g_entry_point);
	...
	pthread_mutex_unlock(&table->forks[min(philosopher->right_fork,
	philosopher->left_fork)]);
	pthread_mutex_unlock(&table->forks[max(philosopher->right_fork,
	philosopher->left_fork)]);
