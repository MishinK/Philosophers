...
t_table				*ft_init_table(t_data *data)
{
	...
	if (!(table = (t_table *)malloc(sizeof(t_table) * 1)))
		printf("error malloc\n");
	sem_unlink("forks");
	table->forks = sem_open("forks", O_CREAT, 0777, data->number_of_phil);
	if (table->forks == (sem_t *)SEM_FAILED || table->forks == NULL)
		printf("error forks\n");
	return (table);
}
...
t_philosopher		**ft_init_all_philosopher(t_data *data)
{
	...
	* data->number_of_phil)))
	...
	while (i < data->number_of_phil)
	{
		...
		str = ft_itoa(i);
		sem_unlink(str);
		philo[i]->life_mutex = sem_open(str, O_CREAT, 0777, 1);
		free(str);
		i++;
	}
	return (philo);
